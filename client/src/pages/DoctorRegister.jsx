  ...
  const formik = useFormik({
    ...
    onSubmit: async values => {
      if (values.password !== values.retype) {
        ...
        try {
          response = await axios.post("http://localhost:4000/api/v1/doctorlist/doctorregister", values);
        ...
        <div className="flex flex-wrap">
          ...
          <div className="w-full lg:w-6/12 px-4">
            <div className="relative w-full mb-3">
              ...
              <input  {...formik.getFieldProps('specialization')} type="text" className="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" placeholder='Enter you surname' required />
            </div>
          </div>
          <div className="w-full lg:w-6/12 px-4">
            <div className="relative w-full mb-3">
